...
function groupPage(d, kind, id) {
  ...
  return shell(`<nav class="crumbs"><a href="/">Home</a> / <a href="/${kind === "agency" ? "agencies" : "categories"}">${kind === "agency" ? "Agencies" : "Categories"}</a> / ${esc(name)}</nav><p class="kicker">Current opportunity collection</p><h1>${esc(name)}</h1><p class="lead">${matches.length} current opportunit${matches.length === 1 ? "y" : "ies"}, ordered by published deadline.</p>${cards(matches)}`, { title: `${name} current grants | Where Are The Grants`, description: `Current Grants.gov opportunities for ${name}.`, canonical: `https://wherearethegrants.com/${kind}/${route(id)}` });
}
function directory(d, kind) {
  ...
  return shell(`<p class="kicker">Browse current calls</p><h1>Funding ${plural}</h1><p class="lead">Stable pages built only from current opportunities in the approved snapshot.</p><section class="grid">${rows.map(([id, r]) => `<a class="card" href="/${kind}/${route(id)}"><small>${r.count} current</small><h2>${esc(r.name)}</h2></a>`).join("")}</section>`, { title: `US federal funding ${plural} | Where Are The Grants`, canonical: `https://wherearethegrants.com/${plural}` });
}
function sitemap(d) {
  const paths = ["/", "/opportunities", "/agencies", "/categories", "/sources"], agencies = new Set(), categories = new Set();
  for (const r of d.records) { paths.push(`/grant/${route(r.OpportunityID)}`); if (r.AgencyCode) agencies.add(r.AgencyCode); if (r.CategoryOfFundingActivity) categories.add(r.CategoryOfFundingActivity); }
  for (const id of agencies) paths.push(`/agency/${route(id)}`); for (const id of categories) paths.push(`/category/${route(id)}`);
...
export default { async fetch(request, env) {
  ...
  else if (url.pathname === "/sources") body = shell(`<p class="kicker">Source and boundary</p><h1>What this site publishes</h1><section class="panel"><h2>${esc(d.sourceSnapshot)}</h2><p>${esc(d.rightsNote)}</p><p><strong>Personal-data rule:</strong> ${esc(d.personalDataPolicy)}</p><p>${esc(d.archivedRecordsExcluded)} archived or closed records are excluded.</p><p><a href="${esc(d.sourceUrl)}">Grants.gov search ↗</a> · <a href="${esc(d.licenceEvidenceUrl)}">Copyright guidance ↗</a></p></section>`, { title: "Sources and method | Where Are The Grants", canonical: "https://wherearethegrants.com/sources" });
